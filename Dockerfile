...
# build runc (PIE = Position Independent Executable)
FROM buildbase_go AS runc_pie
ARG JOBS
ARG RUNC_VERSION
...
ARG JOBS
ARG CRUN_VERSION
RUN git clone -c 'advice.detachedHead=false' --depth=1 --branch="$CRUN_VERSION" https://github.com/containers/crun.git /crun
WORKDIR /crun
RUN set -ex; \
    ./autogen.sh && \
...
# podman build --jobs 12 .
# podman build --target rootlesspodmanbase -t rootlesspodmanbase:latest .
# podman build --target rootlesspodmanminimal -t rootlesspodmanminimal:latest .
# podman build --target podmanall -t podmanall:latest .
# podman build --target tar-archive -t tar-archive:latest .
